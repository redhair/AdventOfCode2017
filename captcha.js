...
(function(){
	...
	function solveCaptcha(){
		for(let i=0;i<context.input.length;++i){
			if(i===0){
				handleFirstDigit(parseInt(context.input[i]));
			}

			if(i===context.input.length-1){
				handleLastDigit(parseInt(context.input[i]));
			}

			if(parseInt(context.input[i])==parseInt(context.input[i+1])){
				context.sum+=parseInt(context.input[i]);
			}
		}
		return context.sum;
	}

	function handleFirstDigit(digit){
		context.first=digit;
	}

	function handleLastDigit(digit){
		context.last=digit;

		if(context.first!=undefined){
			if(context.first==context.last){
				context.sum+=context.last;
			}
		}
	}
})();