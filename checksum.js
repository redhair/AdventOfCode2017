...
(function(){
	...
	function _init(){
		...
		for(let i=0;i<context.processStack.length;++i){
			context.checksum+=getMinMaxSum(context.processStack[i]);
		}
	...
	function getMin(line){
		...
		for(let i=0;i<numbers.length;++i){
			if(parseInt(numbers[i])<min||min===undefined){
				min=numbers[i];
			}
		}
	...
	function getMax(line){
		...
		let numbers=line.split("\t");
		for(let i=0;i<numbers.length;++i){
			if(parseInt(numbers[i])>max||max===undefined){
				max=numbers[i];
			}
		}
